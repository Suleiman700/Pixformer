...
function paginator(config) {
    ...
    if (typeof config.box_mode == "function") {
        ...
        for (var i = 1; i <= pages; i++) {
            if (i == 1 || i == 2 || i == page || i == pages - 1 || i == pages) {
                var li = make_button(i, i, config, false, (page == i));
                page_box.appendChild(li);
            }
            if (pages > 3 && (i == pages - 3 || i == 3)) { //&& (page > pages - 2 && page > 3)
                var li = document.createElement("li");
                li.innerHTML = " . . . ";
                page_box.appendChild(li);
            }
    ...
    let Trlength = trs.length/2;
    // console.log("pageee");

    // console.log(Trlength);
    var stat = document.createElement("span");
    stat.innerHTML = "בעמוד " + page + " מתוך " + pages +
        ", מציג שורות " + (((page - 1) * rows_per_page / 2) + 1) +
        " עד " + (trs.length < page * rows_per_page || rows_per_page == 0 ? trs.length : page * rows_per_page) +
        // " עד " + (trs.length < page * rows_per_page || rows_per_page == 0 ? trs.length : page * rows_per_page) / 2 +
        // " מתוך " + trs.length / 2;
        " מתוך " + Trlength ;
